...
// WrapRequest WriteToRequest calls the nested upgrader and then copies the returned request
...
func (h *UpgradeAwareHandler) ServeHTTP(w http.ResponseWriter, req *http.Request) {
	...
	proxyPrefix := req.Header.Get("X-Proxy-Prefix")
	req.Header.Del("X-Proxy-Prefix")
	if strings.HasPrefix(newReq.URL.Path, proxyPrefix) {
		if proxyPrefix != h.Location.Path {
			newReq.URL.Path = strings.TrimPrefix(newReq.URL.Path, proxyPrefix)
		}
	}
